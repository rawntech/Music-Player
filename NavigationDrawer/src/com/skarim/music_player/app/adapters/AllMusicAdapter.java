...
public class AllMusicAdapter extends ArrayAdapter<Song> {

	private ArrayList<Song> allMusicList;
	Context context;

	public AllMusicAdapter(Context _context, int textViewResourceId,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			viewHolder.cover = (ImageView) convertView
					.findViewById(R.id.ivCoverImage);
		...
		if (allMusicList.get(position).getSongName().length() > 18) {
			String d = allMusicList.get(position).getSongName()
					.substring(0, 18);
			viewHolder.songName.setText("" + d + "");
		} else {
			viewHolder.songName.setText(""
					+ allMusicList.get(position).getArtistName());
		}

		if (allMusicList.get(position).getArtistName().length() > 18) {
			String d = allMusicList.get(position).getArtistName()
					.substring(0, 18);
		...
		if (allMusicList.get(position).getDuration() != null) {
			int dur = Integer
					.parseInt(allMusicList.get(position).getDuration());
		...
		if (allMusicList.get(position).getCoverPath() == null) {
			viewHolder.cover.setImageResource(R.drawable.album_img_128);
		} else {
			viewHolder.cover.setImageBitmap(allMusicList.get(position).getCover());
		}
